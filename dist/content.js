...
bandar togel online
bandar slot online
`.split(/\r?\n/).map(e=>e.trim()).filter(Boolean),y=new Map,b=new Map,x=new Map;[`Boyer-Moore`,`KMP`,`Regex`,`Weighted-Levenshtein`].forEach(e=>{b.set(e,0),x.set(e,0)});var S=document.createElement(`div`);S.style.position=`absolute`,S.style.display=`none`,S.style.backgroundColor=`#333`,S.style.color=`#fff`,S.style.padding=`8px`,S.style.borderRadius=`4px`,S.style.fontSize=`12px`,S.style.zIndex=`999999`,S.style.pointerEvents=`none`,S.style.lineHeight=`1.5`,document.body.appendChild(S);function C(e,t,n){let r=0,i=[];for(let e of t){let t=b.get(e.algorithm)||0;b.set(e.algorithm,t+e.stats.executionTimeMs);let n=x.get(e.algorithm)||0;x.set(e.algorithm,n+e.matches.length);for(let t of e.matches)i.push({...t,algorithms:new Set([e.algorithm])})}if(i.length>0){i.sort((e,t)=>e.startIndex-t.startIndex);let t=[];for(let e of i){let n=t[t.length-1];n&&n.startIndex===e.startIndex&&n.endIndex===e.endIndex?e.algorithms.forEach(e=>n.algorithms.add(e)):t.push(e)}let a=[],o=-1;for(let e of t)e.startIndex>=o&&(a.push(e),o=e.endIndex);if(a.length===0)return 0;let s=document.createDocumentFragment(),c=0;for(let t of a){let n=e.slice(c,t.startIndex);n&&s.appendChild(document.createTextNode(n));let i=document.createElement(`mark`);i.className=`sweetbonanza-highlighted-word`,i.style.backgroundColor=`red`,i.style.color=`white`,i.style.cursor=`help`;let a=e.slice(t.startIndex,t.endIndex);i.textContent=a;let o=a.toLowerCase();y.set(o,(y.get(o)||0)+1),i.dataset.keyword=a,i.dataset.algorithms=Array.from(t.algorithms).join(`, `),s.appendChild(i),c=t.endIndex,r++}let l=e.slice(c);l&&s.appendChild(document.createTextNode(l)),n.parentNode?.replaceChild(s,n)}return r}function w(){let e=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT,{acceptNode:e=>{if(!e.nodeValue?.trim())return NodeFilter.FILTER_REJECT;let t=e.parentNode;return t&&[`SCRIPT`,`STYLE`,`NOSCRIPT`,`MARK`].includes(t.nodeName)?NodeFilter.FILTER_REJECT:NodeFilter.FILTER_ACCEPT}}),t=[],r=e.nextNode();for(;r;)t.push(r),r=e.nextNode();let i=0;for(let e of t){let t=e.nodeValue||``,r=[n(t,v),a(t,v),c(t),_(t,v)];i+=C(t,r,e)}document.querySelectorAll(`.sweetbonanza-highlighted-word`).forEach(e=>{let t=e;t.addEventListener(`mouseenter`,e=>{let n=e,r=t.dataset.keyword||``,i=r.toLowerCase(),a=y.get(i)||0,o=t.dataset.algorithms||``,s=``;o.split(`, `).forEach(e=>{if(!e)return;let t=b.get(e)||0;s+=`- ${e}: ${t.toFixed(3)} ms<br>`}),S.innerHTML=`
            ...
            ${s}
          `,S.style.display=`block`,S.style.left=`${n.pageX+15}px`,S.style.top=`${n.pageY+15}px`}),t.addEventListener(`mousemove`,e=>{let t=e;S.style.left=`${t.pageX+15}px`,S.style.top=`${t.pageY+15}px`}),t.addEventListener(`mouseleave`,()=>{S.style.display=`none`})}),console.log(`Pencarian selesai. Ditemukan ${i} kata unik.`),console.log(`Statistik Waktu per algoritma:`,Object.fromEntries(b))}w()})();